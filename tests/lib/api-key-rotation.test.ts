...
describe('rotateApiKey', () => {
  ...
  it.each<[24 | 168 | 720]>([[24], [168], [720]])(
    'issues a new key and sets expires_at correctly for graceHours=%i',
    async (graceHours) => {
      const old = await makeKey('production');
      const { fullKey, prefix } = await rotateApiKey({ apiKeyId: old.id, graceHours });
      ...
      const expected  = Date.now() + graceHours * 3600 * 1000;
      expect(Math.abs(expiresAt - expected)).toBeLessThan(60_000);

      const { data: newRow } = await admin.from('api_keys')
        .select('id, name, merchant_id, revoked_at, expires_at')
        .eq('id', oldRow!.rotated_to)
        .single();
      expect(newRow!.name).toBe('production');
      expect(newRow!.merchant_id).toBe(MERCHANT_ID);
  ...
  it('refuses to rotate a non-existent key id', async () => {
    const phantom = '00000000-0000-0000-0000-0000000000ff';
    await expect(rotateApiKey({ apiKeyId: phantom, graceHours: 24 })).rejects.toThrow(/not found/);
  });
});
